...
IMAGE_ID="bouquet"
IMAGE_PRETTY_NAME="Bouquet"
IMAGE_LIKE="fedora"
HOME_URL="https://github.com/LumpBloom7/bouquet"
DOCUMENTATION_URL="https://github.com/LumpBloom7/bouquet"
SUPPORT_URL="https://github.com/LumpBloom7/bouquet/issues"
BUG_SUPPORT_URL="https://github.com/LumpBloom7/bouquet/issues"

if grep -q "VARIANT_ID" /usr/lib/os-release; then
    sed -i "s/^VARIANT_ID=.*/VARIANT_ID=$IMAGE_ID/" /usr/lib/os-release
else
    echo "VARIANT_ID=$IMAGE_ID" >> /usr/lib/os-release
fi

if grep -q "VARIANT" /usr/lib/os-release; then
    sed -i "s/^VARIANT=.*/VARIANT=$IMAGE_PRETTY_NAME/" /usr/lib/os-release
else
    echo "VARIANT=$IMAGE_PRETTY_NAME" >> /usr/lib/os-release
fi
